...
const snake = new Snake('sn') // 继承
snake.run() // 继承

const horse = new Horse('ho') // 继承
horse.run() // 继承

// 多態：父類型的引用指向子類型的對象，
// 不同類型的對象針對相同的方法，產生了不同的行爲
// 父类型引用指向子类型的對象 ==> 多态
...
console.log(new Student('abc').sex) //  受保护的不可见
console.log(new Person('abc').run)
console.log(new Student('abc').run)
...
//  在这个例子里，我们使用 static 定义 origin，
//  因为它是所有网格都会用到的属性。 
//  每个实例想要访问这个属性的时候，
//  都要在 origin 前面加上类名。
//   如同在实例属性上使用 this.xxx 来访问属性一样，
//   这里我们使用 Grid.xxx 来访问静态属性。

//  通过类名.静态属性的方式来设置该成员数据
//  通过类名.静态属性的方式来读取该成员数据
...
console.log(Personr.name2) // 可讀
Personr.name2 = 'xixi' // 可寫
Personr.sayHi()
console.log(Personr.name2)
console.log(new Personr().name1)
